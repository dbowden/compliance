...
data <- subset(data, select=c(dyad,year,DispNum3,claimdy))

#can't find a good/reliable way to do reverse lags in panel data in R, so we'll export to Stata for this step
write.csv(data,"pre_stata_slim.csv")

#see stata do file "data_creation_2.do" for commands, then continue to data_creation_3.R